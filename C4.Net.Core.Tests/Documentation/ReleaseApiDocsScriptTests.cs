...
namespace C4.Net.Core.Tests.Documentation
{
	...
	public class ReleaseApiDocsScriptTests
	{
		...
		public void Main_WritesReleaseNotes_WhenInvocationIsValid()
		{
			...
			Assert.Contains("restore C4.Net.slnx -p:TargetFramework=net10.0", dotnetLog);
		...
		public void WriteReleaseNotes_WritesExpectedContent()
		{
			...
			try
			{
				...
				Assert.Contains("https://www.nuget.org/packages/C4.Net.Core/1.2.3", content);
				Assert.Contains("https://www.nuget.org/packages/C4.Net.Editor/1.2.3", content);
		...
		private static TestScriptWorkspace CreateTestScriptWorkspace()
		{
			...
			File.WriteAllText(Path.Combine(rootPath, "C4.Net.slnx"), "<Solution />");
